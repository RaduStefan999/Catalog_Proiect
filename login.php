...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($email_err) && empty($password_err)){
        ...
        $sql = "SELECT id, email, password FROM profesori WHERE email = ?";
...
<div class="login_container">
  <form class="form-signin" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">       
    ...
    <?php 
      if(!empty($login_err)){
          echo '<div class="login_errors">' . $login_err . '</div>';
      } 
      if(!empty($email_err)){
        echo '<div class="login_errors">' . $email_err . '</div>';
      }
      if(!empty($password_err)){
        echo '<div class="login_errors">' . $password_err . '</div>';
      }   
    ?>
